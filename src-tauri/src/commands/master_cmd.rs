...
pub fn list_master_bank(state: State<DbState>) -> Result<Vec<MasterItem>, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, nama, kode, is_active FROM master_bank ORDER BY nama")
...
pub fn list_master_merek(state: State<DbState>) -> Result<Vec<MasterItem>, String> {
    ...
    let mut stmt = conn
        .prepare(
            "SELECT id, nama, NULL, is_active FROM master_merek WHERE is_active=1 ORDER BY nama",
        )
        .map_err(|e| e.to_string())?;
    let rows = stmt
        .query_map([], |row| {
...
pub fn create_master_bank(
    ...
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "INSERT INTO master_bank (nama, kode) VALUES (?1, ?2)",
        params![nama, kode],
    )
    .map_err(|e| e.to_string())?;
...
pub fn create_master_ekspedisi(state: State<DbState>, nama: String) -> Result<i64, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    conn.execute(
        "INSERT INTO master_ekspedisi (nama) VALUES (?1)",
        params![nama],
    )
    .map_err(|e| e.to_string())?;
    Ok(conn.last_insert_rowid())
...
pub fn create_master_merek(state: State<DbState>, nama: String) -> Result<i64, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    conn.execute("INSERT INTO master_merek (nama) VALUES (?1)", params![nama])
        .map_err(|e| e.to_string())?;
    Ok(conn.last_insert_rowid())
}
